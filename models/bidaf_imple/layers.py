...
class Embedding(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_size, drop_prob, pretrained_embedding=None):
        ...
        if pretrained_embedding is None:
            self.w_embed = nn.Embedding(w_vocab_size, w_emb_size)
        else:
            import numpy as np
            embedding = np.load(pretrained_embedding)
            embedding = torch.Tensor(embedding)
            self.w_embed = nn.Embedding.from_pretrained(embedding, freeze=True)
            print("word embedding is pretrained")
        self.c_embed = nn.Embedding(c_vocab_size, c_emb_size)
        self.proj = nn.Linear(w_emb_size, hidden_size, bias=False)
        self.char_conv = nn.Conv2d(1, self.char_channel_size, (self.char_embedding_size, self.char_channel_width))
    ...
    def forward(self, x, y):
        ...
        c_emb = c_emb.view(-1, self.char_embedding_size, c_emb.size(2)).unsqueeze(1)
        c_emb = self.char_conv(c_emb).squeeze()
        c_emb = F.max_pool1d(c_emb, c_emb.size(2)).squeeze()
        c_emb = c_emb.view(batch_size, -1, self.char_channel_size)
