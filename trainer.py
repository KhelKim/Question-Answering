...
class Trainer(object):
    ...
    def train(self, epochs, batch, print_iter, save_model_dir, run_name):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                self.model.eval()
                for iter_, data in enumerate(self.val_loader):
                    logits, loss = self.forward(data, self.model, self.loss_fn, self.device)
                    metric = self.get_metric(logits, data)
                ...
                if val_loss.avg < monitor_value:
                    self.save_model(save_model_dir, run_name, self.model, self.optimizer)
    ...
    def save_model(self, directory, name, model, optimizer):
